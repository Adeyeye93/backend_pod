...
#     Logger.warning(
#       "Unknown audio frame format: #{inspect(binary_part(payload, 0, min(4, byte_size(payload))), base: :hex)}"
#     )
...
#     Logger.debug(
#       "ADTS header: #{inspect(<<byte0, byte1, byte2, byte3, byte4, byte5, byte6>>, base: :hex)}"
#     )
...
defmodule Pod.BroadcasterSupervisor.Handler.AudioFrame do
      ...
      {:ok, payload, remaining} ->
        case extract_aac_audio(payload, state) do
          ...
          {:frame, frame, new_state} ->
            ...
            case Ingest.AudioBuffer.status(new_state.audio_buffer) do
              {:full, frames} ->
                Logger.info("✓ Buffer full — dispatching #{length(frames)} frames to transcoder")
  ...
  @doc """
  Dispatches buffered frames to a transcoder worker from the pool.

  Replaces the old save_audio_to_file/3. Instead of writing directly to disk,
  we hand the frames off to a pooled FFmpeg worker which re-encodes at 128k,
  192k, and 320k. The result comes back asynchronously as a {:transcoded, result}
  message to the calling Season process (self()).

  Guards:
    - aac_config must be present — without it we cannot build valid ADTS headers
      for FFmpeg and the transcode would produce garbage or fail entirely.
    - If the pool is busy (all 100+ workers occupied) we drop the buffer and log.
      This is intentional for now — backpressure and queuing can be added later.
  """
  def flush_and_transcode(_frames, %{aac_config: nil} = _state) do
    Logger.warning("[AudioFrame] Cannot transcode — AAC config not yet received, dropping buffer")
    :ok
  end
  ...
  def flush_and_transcode(frames, state) do
    case TranscoderPool.checkout() do
      ...
      :busy ->
        Logger.warning(
          "[AudioFrame] All transcoders busy — dropping #{length(frames)} frames for broadcaster #{state.id}"
        )
    end
  end
